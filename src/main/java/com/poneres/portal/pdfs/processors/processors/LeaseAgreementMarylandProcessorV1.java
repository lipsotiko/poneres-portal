...
public class LeaseAgreementMarylandProcessorV1 extends AbstractProcessor {
    ...
    Map<String, FC> pdfFieldsMap() {
        return new HashMap<>() {
            {
                // All fields default to TEXT unless otherwise specified here...
    ...
    public void preProcess(Map<String, Object> metadata) {
        ...
        if (metadata.containsKey("lateFeeNumber")) {
            Double dueWithinDaysNumber = fromObj(metadata.get("lateFeeNumber"));
            Money lateFee = Money.of(CurrencyUnit.USD, dueWithinDaysNumber);
            metadata.put("lateFeeNumber", formatMoney(lateFee));
        }

        if (metadata.containsKey("sendNoticeTo")) {
            String sendNoticeTo = metadata.get("sendNoticeTo").toString();
            if (sendNoticeTo.contains("isLandlord")) {
                metadata.put("isLandlord", true);
                metadata.put("isPropertyManager", false);
            } else if (sendNoticeTo.contains("isPropertyManager")) {
                metadata.put("isLandlord", false);
                metadata.put("isPropertyManager", true);
            }
            metadata.remove("sendNoticeTo");
        }
    }
    ...
    private void setLeaseTerm(Map<String, Object> metadata, LocalDate startDate, LocalDate endDate) {
        if (startDate != null && endDate != null) {
            ...
            String termLength = (years > 0) ? years + " Years" : "";
            termLength += (months > 0) ? " " + months + " Months" : "";
